...
//
// Shared by the desktop app and the eval harness, so both classify generated
// SQL by exactly the same rules. "The model would not do that" is not a
// security model: the app uses this before dry-running a query for
// execution-guided repair, and the harness uses it before executing anything
// against a fixture.
//
// It is one of two layers. The harness pairs it with an engine that cannot
// write (read-only SQLite connection, BEGIN READ ONLY in Postgres). The guard
// can in principle be out-thought; the engine layer catches what it misses.
